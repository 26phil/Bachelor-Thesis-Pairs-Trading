...
Description:
------------
Sector-specific implementation of a cointegration-based pairs trading strategy 
for Euro STOXX 600 constituents. The script executes:

1. **Data Loading & Preprocessing**:
   - Reads daily price data with Bloomberg sector classification 
     from 'EuroStoxx600_Weekdays_Sektor.xlsx'
   - Converts to log prices for statistical testing
   - Filters series with insufficient history

2. **Sector-Based Statistical Tests**:
   - Groups time series by sector
   - Correlation filtering within each sector
   - I(1) classification using Augmented Dickey-Fuller test
   - Engle-Granger cointegration testing with BIC-optimized lag selection

3. **Trading Simulation**:
   - Z-score-based entry and exit rules for each cointegrated pair
   - Capital allocation, transaction costs, and short-selling fee handling
   - Sector-specific portfolio construction

4. **Performance Evaluation**:
   - Annual return, volatility, and Sharpe ratio
   - Maximum drawdown
   - Transaction cost and short fee analysis
...
Input Files:
------------
- EuroStoxx600_Weekdays_Sektor.xlsx : Daily adjusted close prices with sector classification

Output Files:
-------------
- cointegration_results_sector.csv : Cointegrated pairs by sector with regression parameters
...
def load_data_with_sector(filename):
    raw_df = pd.read_excel(filename, header=[0, 1])
    date_series = pd.to_datetime(raw_df.iloc[:, 0])

    price_data = raw_df.iloc[:, 1:]
    price_data.columns.names = ["Ticker", "Sector"]

    price_data.index = date_series
...
price_df, log_df = load_data_with_sector("EuroStoxx600_Weekdays_Sektor.xlsx")
log_df = log_df.loc[:, log_df.notna().sum() >= 300]
price_df = price_df[log_df.columns]


def group_stocks_by_sector(log_df):
    grouped = {}
    for sector in log_df.columns.levels[1]:
        if pd.isna(sector): continue
        sector_cols = [col for col in log_df.columns if col[1] == sector]
        if sector_cols:
            grouped[sector] = log_df[sector_cols]
    return grouped



### Correlation Matrix >= 0.9 ###
def find_high_corr_pairs(log_df, threshold=0.9):
    original_columns = log_df.columns
    simple_cols = [str(col) for col in original_columns]
    log_df.columns = simple_cols
    ...
    corr_pairs["Pair"] = corr_pairs.apply(lambda row: tuple(sorted([row["Stock1"], row["Stock2"]])), axis=1)
    corr_pairs = corr_pairs.drop_duplicates(subset="Pair")
    high_corr_pairs = corr_pairs[corr_pairs["Correlation"] >= threshold].sort_values(by="Correlation", ascending=False)
...
def run_engle_granger_test(log_df, high_corr_pairs, I1_stocks, train_window, crit_value=-3.44):
    cointegrated_pairs = []
...
def calculate_sharpe_ratio(portfolio_series, risk_free_rate_annual):
    n_days = len(portfolio_series)
    
    log_total_return = (portfolio_series.iloc[-1] / portfolio_series.iloc[0]) -1
...
def main():
    # Chose time series path
    filename = "EuroStoxx600_Weekdays_Sektor.xlsx"
    ...
    def clean_ticker(entry):
        if isinstance(entry, str):
            # Remove everything except the actual ticker symbol
            match = re.match(r"\(?['\"]?([A-Z0-9 ]+ EQUITY)['\"]?,?", entry)
            if match:
    ...
    # === Load new file with sectors ===
    price_df, log_df = load_data_with_sector("EuroStoxx600_Weekdays_Sektor.xlsx")

    # === Keep only columns with sufficient data ===
    log_df = log_df.loc[:, log_df.notna().sum() >= 300]
    price_df = price_df[log_df.columns]

    # === Results path ===
    results_path = get_local_filepath("cointegration_results_sector.csv")

    # === Load or calculate cointegration ===
    if os.path.exists(results_path) and os.path.getsize(results_path) > 0:
        print("Loading existing cointegration pairs...")
        cointegration_results = pd.read_csv(results_path)
    else:
        print("Calculating new cointegration pairs...")
        sector_groups = group_stocks_by_sector(log_df)
        all_cointegrated_pairs = []

        for sector, sector_log_df in sector_groups.items():
            print(f"🔍 Sector: {sector} | {len(sector_log_df.columns)} series")

            high_corr_pairs = find_high_corr_pairs(sector_log_df, threshold=0.9)
            I0, I1, invalid = classify_series(sector_log_df)
            coint_pairs = run_engle_granger_test(sector_log_df, high_corr_pairs, I1, train_window = train_window)

            if not coint_pairs.empty:
                # Extract tickers correctly
                coint_pairs["Stock1"] = coint_pairs["Stock1"].apply(clean_ticker)
                coint_pairs["Stock2"] = coint_pairs["Stock2"].apply(clean_ticker)
                coint_pairs["Sector"] = sector
                all_cointegrated_pairs.append(coint_pairs)
            else:
                print(f" No cointegrated pairs found in sector {sector}")

        cointegration_results = pd.concat(all_cointegrated_pairs, ignore_index=True)
        cointegration_results.to_csv(results_path, index=False)
        print(f" Number of cointegrated pairs: {len(cointegration_results)}")

    # === Convert to simple column names (tickers only) ===
    price_df.columns = price_df.columns.get_level_values(0)
    log_df.columns = log_df.columns.get_level_values(0)
    ...
    annual_return, annual_volatility, sharpe_ratio = calculate_sharpe_ratio(
            portfolio_series, 
    ...
    # === Results reports ===
    print("\nSimulation results:")
    print(f"Portfolio value: {portfolio_series.iloc[-1]:,.2f} $")
    ...
    analyze_costs(df_trades, transaction_cost_rate=transaction_cost_rate, initial_capital=initial_capital)
